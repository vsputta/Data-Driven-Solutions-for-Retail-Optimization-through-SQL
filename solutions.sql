...
SELECT 
	  ...
	  c.category_name,
	  SUM(oi.total_sale) as total_sale,
	  SUM(oi.total_sale)/(SELECT SUM(total_Sale) FROM order_item) * 100 as contribution
...
SELECT  
      c.state,
	  ct.category_name,
	  ROUND(SUM(oi.total_sale::numeric),1) as total_sale,
...
SELECT 
	  ...
	  s.shipping_date - o.order_date as dats_took_to_ship
FROM orders as o
JOIN
customers as c
on c.customer_id = o.customer_id
